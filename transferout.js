...
window.openShipmentLobby = function (shipmentNo, branch, status) {
  // 🚨 ล้างกระดานทันทีที่ถูกเรียก ไม่ให้เห็นของเก่าแว็บๆ
  const container = document.getElementById("lobbyContentContainer");
  if (container) container.innerHTML = "";

  window.currentShipmentNo = shipmentNo;
  window.currentShipmentStatus = status || "Assign";
  sessionStorage.setItem(
    "lobbyMode",
    window.currentShipmentStatus.toUpperCase(),
  );

  const title = document.getElementById("lobbyMasterTitle");
  if (title)
    title.innerText = window.currentShipmentStatus.toUpperCase() + " LOBBY";

  const branchHeader = document.getElementById("lobbyBranchHeaderName");
  if (branchHeader) branchHeader.innerText = `[${branch}]`;

  if (typeof window.applyLobbyTheme === "function") window.applyLobbyTheme();

  const draftData = window.getDraftDataForShipment(shipmentNo);
  if (!draftData || draftData.length === 0) {
    container.innerHTML = `
          <div id="lobbyEmptyState" style="text-align: center; padding: 40px 20px; margin: auto;">
              <i class="fas fa-truck-loading" style="font-size: 50px; color: #ddd; margin-bottom: 15px;"></i>
              <p style="color: #888; font-weight: bold;">ไม่มีข้อมูลกล่อง</p>
              ${window.currentShipmentStatus === "Assign" ? '<p style="color: #aaa; font-size: 14px;">กดปุ่ม + เพื่อสร้างกล่อง</p>' : ""}
          </div>`;
  } else {
    draftData.forEach((boxData, index) => {
      container.appendChild(
        window.createShipmentColumn(
          boxData,
          index + 1,
          window.currentShipmentStatus,
        ),
      );
    });
  }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // 🚨 แก้ไขที่ 2.2: ล้างกระดานตอนกดย้อนกลับ
  const btnCancelFromLobby = document.getElementById("btnCancelFromLobby");
  if (btnCancelFromLobby) {
    const newBtnCancel = btnCancelFromLobby.cloneNode(true);
    btnCancelFromLobby.parentNode.replaceChild(
      newBtnCancel,
      btnCancelFromLobby,
    );
    newBtnCancel.addEventListener("click", () => {
      ...
      if (typeof hideLobbyView === "function") hideLobbyView();
      const vTaskHub = document.getElementById("transferOutTaskHubView");
      const vLobby = document.getElementById("transferOutLobbyView");
      if (typeof navigationTo === "function" && vTaskHub && vLobby) {
        navigationTo(vLobby, vTaskHub);
      }
      if (typeof loadExistingTasks === "function") loadExistingTasks();
    });
  }

  document.getElementById("btnBackToTaskHub")?.addEventListener("click", () => {
    if (typeof hideLobbyView === "function") hideLobbyView();
  });
